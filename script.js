...
function showWinModal(game, message, seconds, score) {
  ...
  // Dipaksa aktif (tidak disabled/readonly) supaya nama tetap bisa
  // diisi walaupun jawaban belum semua benar (skor parsial).
...
document.getElementById('win-save').addEventListener('click', async () => {
  ...
  const saved = await saveToLeaderboard(
    winContext.game,
    name,
    winContext.seconds,
    winContext.score
...
function buildCrosswordData() {
  ...
  cwGrid = Array.from(
    { length: cwRows },
    () => Array(cwCols).fill(null)
  );

  cwAcrossMap = Array.from(
    { length: cwRows },
    () => Array(cwCols).fill(null)
  );

  cwDownMap = Array.from(
    { length: cwRows },
    () => Array(cwCols).fill(null)
  );
...
function renderCrosswordClues() {
  ...
  CROSSWORD_WORDS.forEach((w, idx) => {
    ...
    li.innerHTML =
      `<b>${w.num}.</b> ${w.clue} (${w.answer.length} huruf)`;
...
function cwInputEl(r, c) {
  return document.querySelector(
    `#crossword-grid input[data-r="${r}"][data-c="${c}"]`
  );
}
...
function highlightCwWord() {
  ...
  document
    .querySelectorAll('.cw-cell.active')
    .forEach(el => {
      el.classList.remove('in-word', 'current');
    });

  document
    .querySelectorAll('.clue-group li')
    .forEach(li => {
  ...
  const li =
    document.querySelector(
      `.clue-group li[data-idx="${cwCurrentWordIdx}"]`
    );
...
// Mengecek jawaban tanpa menampilkan feedback visual
// dipakai tiap ketikan untuk mendeteksi kemenangan
// secara otomatis & memperbarui skor.
function checkCwSilently() {
  ...
  // Skor dinormalisasi ke 100 agar jumlah soal
  // tidak membuat skor bias.
  cwScore = Math.round(
    (
      correctWords /
      CROSSWORD_WORDS.length
    ) * CW_MAX_SCORE
  );
  ...
  if (
    correctWords === CROSSWORD_WORDS.length &&
    !cwWon
  ) {
...
document
  ...
  .addEventListener('click', () => {
    ...
    document
      .querySelectorAll(
        '#crossword-grid .cw-cell.active'
      )
      .forEach(cellDiv => {

        const input =
          cellDiv.querySelector('input');
    ...
    feedback.textContent =
      `${correct} dari ${total} kotak sudah benar.`;
    ...
    // Tetap izinkan skor parsial masuk leaderboard.
    if (!cwWon) {

      const correctWords =
        CROSSWORD_WORDS.filter(w => {
      ...
      if (
        ...
        showWinModal(
          'crossword',
          `Skor kamu saat ini: ${correctWords} dari ${CROSSWORD_WORDS.length} soal benar.`,
          cwTimer.getSeconds(),
          partialScore
        );
      }
    ...
    // Hilangkan tanda "incorrect"
    // setelah beberapa saat agar bisa dicoba lagi.
    setTimeout(() => {

      document
        .querySelectorAll(
          '#crossword-grid .cw-cell.incorrect'
        )
        .forEach(el => {
...
cwHintBtn.addEventListener(
  ...
  () => {
    ...
    // Isi 1 huruf yang masih kosong/salah pada sebuah kata,
    // lalu pakai 1 jatah hint. Return true kalau berhasil isi.
    function fillHintForWord(idx) {

      const w = CROSSWORD_WORDS[idx];
      if (!w) return false;
    ...
    // 1) Coba isi soal yang sedang dipilih user (mis. nomor 5)
    if (
      cwCurrentWordIdx !== null &&
      fillHintForWord(cwCurrentWordIdx)
    ) {
      return;
    }

    // 2) Soal yang dipilih sudah lengkap/tidak ada yang dipilih
    //    -> fallback: cari soal lain yang masih kosong/salah
    for (
      let r = 0;
      r < cwRows;
      r++
...
const WS_SIZE = 14;
const WS_MAX_ATTEMPTS_PER_WORD = 300;
const WS_MAX_PUZZLE_ATTEMPTS = 40;
...
function generateWordSearchPuzzle() {
  ...
  for (
    let attempt = 0;
    attempt < WS_MAX_PUZZLE_ATTEMPTS;
    attempt++
  ) {
  ...
  console.warn(
    'Gagal membangun puzzle setelah banyak percobaan, mencoba grid darurat.'
  );

  return generateEmergencyPuzzle();
}

// Fallback ekstra-aman
...
function wsCellEl(r, c) {
  ...
  return document.querySelector(
    `#ws-grid .ws-cell[data-r="${r}"][data-c="${c}"]`
  );
}


/* ----------------------------------------------------------------
   3.3 INTERAKSI PEMILIHAN KATA
   ---------------------------------------------------------------- */
...
function finalizeWsSelection(
  ...
  if (match) {
    ...
    const li =
      document.querySelector(
        `#ws-word-list li[data-word="${match.word}"]`
      );
    ...
    // Skor dinormalisasi ke 100
    // agar 14 kata tidak otomatis menjadi 140.
    wsScore =
      Math.round(
        (
          wsFoundWords.size /
    ...
    // Bukan kata yang benar
    // -> beri kedipan merah singkat.
    path.forEach(
      ({ r, c }) => {
...
document
  .getElementById('ws-reset')
  .addEventListener(
    'click',
    resetWordSearchProgress
  );
    ...
    return data
      ...
      .sort(
        (a, b) => {
          ...
          // Kalau skor sama,
          // waktu lebih cepat = lebih tinggi
          return (
            a.seconds -
            b.seconds
          );
